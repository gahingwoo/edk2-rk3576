...
if [ "$BOOT_MEDIUM" = "sdcard" ]; then
  ...
  0x1600000                NV variable store (3 x 64 KB)"
    ...
    # Partitioned eMMC image.  The plain sdcard image is the firmware and
    # nothing else, so a board flashed with it has 29 GiB of eMMC that no
    # filesystem can reach.  This one is the same bytes with a partition table
    # in front: the firmware region declared so nothing is allocated over it,
    # and the remainder of the eMMC as one data partition.
    if [ -n "${EMMC_BYTES:-}" ]; then
        ...
        FW_MB=64
        DATA_START=$(( FW_MB * 1024 * 1024 / 512 ))

        cat > "$LAYOUT_F" <<EOF
# $PLATFORM_NAME eMMC layout, $EMMC_BYTES bytes.  Generated by scripts/package.sh.
# $PLATFORM_NAME-emmc.img already contains this table.  You only need this file
# if you flashed the plain sdcard image, which starts with 32 KB of zeros and so
# wipes the protective MBR and the primary GPT:
#   sudo sfdisk --wipe always /dev/mmcblk0 < $(basename "$LAYOUT_F")
label: gpt
unit: sectors
first-lba: 34

start=64,          size=$(( DATA_START - 64 )), type=8DA63339-0007-60C0-C436-083AC8230908, name="firmware"
start=$DATA_START, type=0FC63DAF-8483-4772-8E79-3D69D8477DE4, name="data"
EOF

        # sfdisk has to see the whole device to place the last partition and
        # size the backup GPT, so lay the table on a sparse file of the real
        # eMMC size and keep only the front of it.
        rm -f "$WORK/full.img"
        truncate -s "$EMMC_BYTES" "$WORK/full.img"
        sfdisk --wipe always "$WORK/full.img" < "$LAYOUT_F" >/dev/null
        # Firmware from sector 64 on, so the table written above survives.
        dd if="$OUT_IMG" of="$WORK/full.img" bs=512 skip=64 seek=64 conv=notrunc status=none
        dd if="$WORK/full.img" of="$EMMC_IMG" bs=1M count=$SD_MB status=none
        ok "$EMMC_IMG  ($(du -h "$EMMC_IMG" | cut -f1))"
