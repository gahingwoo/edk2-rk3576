...
if [ "$BOOT_MEDIUM" = "sdcard" ]; then
  ...
  0x1600000                NV variable store (3 x 64 KB)"
    ...
    # Partitioned eMMC image.  The plain sdcard image above is the firmware and
    # nothing else, so a board flashed with it has 29 GiB of eMMC that no
    # filesystem can reach.  This one carries a GPT, an ESP holding the rescue
    # UKI, and a data partition covering the rest of the device.
    if [ -f "$OUTDIR/rescue.efi" ] && [ -n "${EMMC_BYTES:-}" ]; then
        command -v mcopy >/dev/null || die "mtools missing -- needed to fill the ESP without mounting it"
        command -v sfdisk >/dev/null || die "sfdisk missing"

        EMMC_IMG="$OUTDIR/$PLATFORM_NAME-emmc.img"
        LAYOUT_F="$OUTDIR/$PLATFORM_NAME-emmc.sfdisk"
        RESCUE_SZ=$(stat -c %s "$OUTDIR/rescue.efi")
        # ESP: the UKI plus room to drop a second one beside it, rounded to 4 MiB.
        ESP_MB=$(( (RESCUE_SZ * 2 / 1048576 / 4 + 1) * 4 ))
        [ "$ESP_MB" -ge 128 ] || ESP_MB=128
        FW_MB=64
        ESP_START=$(( FW_MB * 1024 * 1024 / 512 ))
        ESP_SECTORS=$(( ESP_MB * 1024 * 1024 / 512 ))
        DATA_START=$(( ESP_START + ESP_SECTORS ))
        IMG_BYTES=$(( (FW_MB + ESP_MB) * 1024 * 1024 ))

        cat > "$LAYOUT_F" <<EOF
# $PLATFORM_NAME eMMC layout, $EMMC_BYTES bytes.  Generated by scripts/package.sh.
# The image already contains this table; you only need it if you flashed the
# plain sdcard image, which starts with 32 KB of zeros and so wipes the
# protective MBR and the primary GPT:
#   sudo sfdisk --wipe always /dev/mmcblk0 < $(basename "$LAYOUT_F")
label: gpt
unit: sectors
first-lba: 34

start=64,           size=$(( ESP_START - 64 )), type=8DA63339-0007-60C0-C436-083AC8230908, name="firmware"
start=$ESP_START,   size=$ESP_SECTORS, type=C12A7328-F81F-11D2-BA4B-00A0C93EC93B, name="rescue-esp"
start=$DATA_START,  type=0FC63DAF-8483-4772-8E79-3D69D8477DE4, name="data"
EOF

        # sfdisk needs to see the whole device to place p3 and size the backup
        # GPT, so lay the table down on a sparse file of the real eMMC size and
        # keep only the front of it.
        rm -f "$WORK/full.img"
        truncate -s "$EMMC_BYTES" "$WORK/full.img"
        sfdisk --wipe always "$WORK/full.img" < "$LAYOUT_F" >/dev/null

        # ESP, filled without mounting anything.
        rm -f "$WORK/esp.img"
        truncate -s $(( ESP_MB * 1024 * 1024 )) "$WORK/esp.img"
        mkfs.vfat -F32 -n RESCUE-ESP "$WORK/esp.img" >/dev/null
        mmd   -i "$WORK/esp.img" ::/EFI ::/EFI/rescue
        mcopy -i "$WORK/esp.img" "$OUTDIR/rescue.efi" ::/EFI/rescue/rescue.efi

        dd if="$WORK/esp.img" of="$WORK/full.img" bs=512 seek="$ESP_START" conv=notrunc status=none
        # Firmware from sector 64 on, so the GPT written above survives.
        dd if="$OUT_IMG" of="$WORK/full.img" bs=512 skip=64 seek=64 conv=notrunc status=none
