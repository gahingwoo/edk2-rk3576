...
STATIC RK3576_MEMORY_REGION_INFO  VirtualMemoryInfo[MAX_VIRTUAL_MEMORY_MAP_DESCRIPTORS];
  ...
  )
{
  ...
  VirtualMemoryInfo[Index].Type          = RK3576_MEM_UNMAPPED_REGION;
  ...
  if ((VariablesBase + VariablesSize) > 0x08400000) {
    ...
    // running firmware volume or the variable store.
    {
      ...
      VirtualMemoryInfo[Index].Type          = RK3576_MEM_BASIC_REGION;
  ...
  VirtualMemoryTable[Index].Attributes   = ARM_MEMORY_REGION_ATTRIBUTE_WRITE_BACK;
  VirtualMemoryInfo[Index].Type          = RK3576_MEM_RUNTIME_REGION;
  ...
  )
{
  ...
  if (!VirtualMemoryInfoInitialized) {
    DEBUG ((
      DEBUG_ERROR,
      "ArmPlatformGetVirtualMemoryMap must be called before Rk3576PlatformGetVirtualMemoryInfo.\n"
      ));
    return;
  }
