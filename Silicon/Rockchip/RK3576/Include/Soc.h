 ...
 * only the base addresses differ.  We pull in the shared register
 * header for the typedefs / register-bit macros, then `#undef` the
 * few base addresses it carries that clash with RK3576's own map
 * before re-defining them below.
 * --------------------------------------------------------------- */
#include <RockchipIpRegs.h>

/* Base macros carried by RockchipIpRegs.h that collide with RK3576's
 * memory map.  They are `#undef`'d here and re-defined further down
 * with the RK3576 addresses, so the instance macros (FSPI, UART0,
 * CRU, ...) declared there end up pointing at the right MMIO window.
 *
 * This header is the ONLY place these five get corrected.  Any module
 * that resolves <Soc.h> to something else gets the uncorrected values,
 * which is how the display stack used to end up with RK3588's CRU.
 */
#undef CRU_BASE
#undef UART0_BASE
#undef UART2_BASE
 ...
 * Clock and reset IDs.
 *
 * WARNING: these are RK3588's, not RK3576's.  They are the indices into
 * the Clocks[]/Resets[] tables in Rk3576CruLib.c, which is itself still
 * RK3588's clock tree -- see the warning at the top of that file.  They
