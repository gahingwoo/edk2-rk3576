/** @file
 *
 *  ACPI platform driver for RK3576
 *
 *  Derived from Silicon/Rockchip/RK3588/Drivers/AcpiPlatformDxe/AcpiPlatformDxe.c
 *  Adapted for RK3576: uses RK3576_MCFG_TABLE, RK3576 PCIe addresses, and
 *  RK3576-specific DSDT device status fixups.
 *
  ...
  )
{
  ...
  //
  // RK3576 has 2 PCIe segments; each is active only when its ComboPHY is in
  // PCIe mode.  Disable the ACPI Device for any segment whose PHY is not PCIe.
  //
  struct {
    CHAR8      *ObjectPath;
    BOOLEAN    Enabled;
  } DevStatus[] = {
    { "\\_SB.PCI0._STA", PcdGet32 (PcdComboPhy0Mode) == COMBO_PHY_MODE_PCIE },
    { "\\_SB.PCI1._STA", PcdGet32 (PcdComboPhy1Mode) == COMBO_PHY_MODE_PCIE },
  };
  ...
  )
{
  ...
  switch (PcieEcamMode) {
    case ACPI_PCIE_ECAM_COMPAT_MODE_NXPMX6:
      ...
      // distinct to the SMMU & ITS (see Rk3576Pcie.h). However, these features
  ...
  if (!McfgSplitConfigSpaces) {
    McfgTable->Header.Header.Length = OFFSET_OF (RK3576_MCFG_TABLE, ConfigSpaces[1]);
  }
...
EFI_STATUS
EFIAPI
RK3576AcpiPlatformDxeInitialize (
