...
// All traces emitted at DEBUG_INFO so they survive RELEASE builds.
//
...
//
// RK3576 has 3 video ports (vs 4 on RK3588) and a smaller window set
// (2 cluster + 4 esmart). The window/PD/DSC tables are reused from
// the RK3588 layout where they overlap. NOTE: Many register offsets
// inside this driver are RK3588-specific and the RK3576 hardware
// layout differs; this table is the minimum needed to compile and
  ...
  )
{
  ...
  if (OutputIf & VOP_OUTPUT_IF_HDMI0) {
    /*
     * RK3576: program a single per-port IF_CTRL register. There is no
     * vop_grf / vo1_grf to write — sync polarity, mux, dividers and
     * enable all live in this 32-bit register. Mirrors mainline
     * rk3576_set_intf_mux() for the EP_HDMI0 case.
     */
    {
      UINT32  IfCtrl;
       ...
       * pixel_rate == 2 so PCLK_DIV stays 0. The legacy IfPixclkDiv variable
       * encodes RK3588 divider values (0..3) and must NOT be used here.
  ...
  )
{
  ...
  if (!RockchipVop2) {
    ...
    //
    // RK3576 has no SYS_PMU window at the address the shared register header
    // carries (0xFD8D8000, which is RK3588's and is ordinary DRAM here), and
    // nothing on this SoC may dereference it — see Vop2WaitPowerDomainOn,
    // which skips the BISR path entirely.  Left as 0 so a future accidental
    // use faults at a recognisable address rather than silently writing DRAM.
    //
    RockchipVop2->SysPmu      = 0;
    RockchipVop2->Version     = mVop2RK3576.Version;
    RockchipVop2->Data        = &mVop2RK3576;
    VOP2_TRACE ("Vop2PreInit: SOC=RK3576 base=0x%x sys_pmu=0x%x version=0x%x\n",
                RockchipVop2->BaseAddress, RockchipVop2->SysPmu, RockchipVop2->Version);
    VOP2_TRACE ("Vop2PreInit: VOP2 register window snapshot\n");
    VOP2_DUMP_REG ("REG_CFG_DONE        ", RockchipVop2->BaseAddress + RK3568_REG_CFG_DONE);
    VOP2_DUMP_REG ("SYS_CTRL0           ", RockchipVop2->BaseAddress + 0x0008);
    VOP2_DUMP_REG ("VP0_DSP_CTRL        ", RockchipVop2->BaseAddress + RK3568_VP0_DSP_CTRL);
    VOP2_DUMP_REG ("VP0_CLK_CTRL        ", RockchipVop2->BaseAddress + RK3588_VP0_CLK_CTRL);
    VOP2_DUMP_REG ("HDMI0_IF_CTRL       ", RockchipVop2->BaseAddress + RK3576_HDMI0_IF_CTRL);
  ...
  VOP2_TRACE ("Vop2PreInit: VP%u max=%ux%u feature=0x%x\n",
              CrtcState->CrtcID,
              CrtcState->MaxOutput.Width, CrtcState->MaxOutput.Height,
              CrtcState->Feature);

  /* fix the plane mask about */
  Vop2GlobalInitial (DisplayState);

  VOP2_TRACE ("Vop2PreInit: EXIT\n");
  return EFI_SUCCESS;
}
  ...
  )
{
   ...
   */
  /*
   * RK3576: the DCLK_VOP2_SEL_CLK_HDMIPHY_PIXEL0_O / DCLK_VOP2 clock IDs
   * above are RK3588-specific (CruLib for RK3576 has a different mux/ID
   * table). Mainline binds clk_vp0 to clk_hdmi0_pixclk via DT clock
   * assignments, which the SPL applies before handoff. Skip the runtime
   * mux switch here and rely on the inherited parent.
   * KNOWN RISK: if the SPL leaves DCLK_VOP0 sourced from a non-HDMI PLL,
   * pixel clock accuracy at 4K@60 may suffer until CRU programming is
   * implemented for RK3576.
   */
  ...
  )
{
  ...
  VOP2_TRACE ("Vop2DxeInitialize: SOC=RK3576 NrVps=%u NrLayers=%u NrMixers=%u NrDscs=%u\n",
              ...
              mVop2RK3576.NrMixers, mVop2RK3576.NrDscs);
  ...
  VOP2_TRACE ("Vop2DxeInitialize: InstallProtocol(CrtcProtocol) -> %r handle=%p\n",
              Status, Handle);
