  ...
  )
{
  ...
  AsciiStrCpyS (
    mBoardInfoProductName,
    sizeof (mBoardInfoProductName),
    (AsciiStrCmp (BoardName, "Unknown") == 0) ? PlatformName : BoardName
    );

  AsciiStrCpyS (
    mBoardInfoManufName,
    sizeof (mBoardInfoManufName),
  ...
  )
{
  ...
  if (Rate < 1000000) {
    // RK3576 has no SCMI; ProcessorGetRate() returns the static fallback
    // (in Hz). For UI display purposes assume the A72 cluster max of
    // 2208 MHz (matches mainline rk3576-cluster1 opp-2208000000).
    Rate = 2208000000;
  }
  ...
  )
{
  ...
  //
  // Source the total system memory size from HOB resource descriptors.
  // The HOB list is established by PrePi and is reliable at this point;
  // the DxeCore GCD has not necessarily promoted all ranges to
  // EfiGcdMemoryTypeSystemMemory yet when this driver runs.
  // SdramGetMemorySize() reads PMU1_GRF OS_REGs which on RK3576 may
  // return 0 in the firewalled silicon revision, so it is only a fallback.
  //
  {
    ...
    for (Hob.Raw = GetHobList (); !END_OF_HOB_LIST (Hob); Hob.Raw = GET_NEXT_HOB (Hob)) {
      if ((GET_HOB_TYPE (Hob) == EFI_HOB_TYPE_RESOURCE_DESCRIPTOR) &&
        ...
        mMemorySize += Hob.ResourceDescriptor->ResourceLength;
      }
    }
