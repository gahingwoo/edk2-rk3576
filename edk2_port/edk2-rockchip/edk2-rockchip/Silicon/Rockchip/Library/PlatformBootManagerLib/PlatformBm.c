  ...
  )
{
  ...
  //
  // PlatformSmbiosDxe runs before DxeCore has promoted all PrePi resource
  // HOBs into GCD SystemMemory; its mMemorySize is sourced from the HOBs
  // directly. As a safety net, recompute the total from GCD here (which is
  // fully populated by AfterConsole) and patch any Type 17 / Type 19 records
  // that are still zero (e.g. SDRAM PCD path returned 0 on this silicon).
  //
  {
    ...
    EFI_SMBIOS_TYPE                  TableType;
    SMBIOS_TABLE_TYPE17              *T17;
    SMBIOS_TABLE_TYPE19              *T19;
    UINT64                           StartKb;
    UINT64                           EndKb;

    if (!EFI_ERROR (gDS->GetMemorySpaceMap (&NumDescriptors, &MemSpaceMap)) &&
        (MemSpaceMap != NULL)) {
      for (i = 0; i < NumDescriptors; i++) {
        if (MemSpaceMap[i].GcdMemoryType == EfiGcdMemoryTypeSystemMemory) {
          Total += MemSpaceMap[i].Length;
        }
      }
    ...
    if ((Total != 0) &&
        !EFI_ERROR (gBS->LocateProtocol (&gEfiSmbiosProtocolGuid, NULL, (VOID **)&Smbios))) {
      Handle    = SMBIOS_HANDLE_PI_RESERVED;
      TableType = SMBIOS_TYPE_MEMORY_DEVICE;
      while (!EFI_ERROR (Smbios->GetNext (Smbios, &Handle, &TableType, &Record, NULL))) {
        T17 = (SMBIOS_TABLE_TYPE17 *)Record;
        if ((T17->Size == 0) || (T17->Size == 0xFFFF)) {
          if (Total / (1024 * 1024) < 0x8000) {
            T17->Size = (UINT16)(Total / (1024 * 1024));
          } else {
            T17->Size         = 0x7FFF;
            T17->ExtendedSize = (UINT32)(Total / (1024 * 1024));
          }
          T17->VolatileSize = Total;
        }
      }

      Handle    = SMBIOS_HANDLE_PI_RESERVED;
      TableType = SMBIOS_TYPE_MEMORY_ARRAY_MAPPED_ADDRESS;
      StartKb   = 0;
      EndKb     = (Total / 1024) - 1;
      while (!EFI_ERROR (Smbios->GetNext (Smbios, &Handle, &TableType, &Record, NULL))) {
        T19 = (SMBIOS_TABLE_TYPE19 *)Record;
        if ((T19->StartingAddress == 0) && (T19->EndingAddress == 0)) {
          if (EndKb <= 0xFFFFFFFF) {
            T19->StartingAddress = (UINT32)StartKb;
            T19->EndingAddress   = (UINT32)EndKb;
          } else {
            T19->StartingAddress         = 0xFFFFFFFF;
            T19->EndingAddress           = 0xFFFFFFFF;
            T19->ExtendedStartingAddress = StartKb * 1024;
            T19->ExtendedEndingAddress   = (StartKb + Total / 1024 - 1) * 1024;
          }
        }
      }
    }
