...
//
// Heavy debug instrumentation tags for the RK3576 HDPTX PHY bring-up.
//
  ...
  )
{
  ...
  PHY_TRACE ("RopllCmnConfig: ENTRY id=%u BitRate=%u\n", Hdptx->Id, BitRate);
  ...
  if (Cfg->Bit_Rate == ~0) {
    PHY_TRACE ("RopllCmnConfig: BitRate %u not in table, computing dynamically\n", BitRate);
    if (HdptxPhyClkPllCalc (BitRate, &Rc)) {
      Cfg = &Rc;
      PHY_TRACE ("RopllCmnConfig: dynamic config OK\n");
    } else {
      DEBUG ((DEBUG_ERROR, "%a: Couldn't find RoPllConfig!\n", __func__));
      PHY_TRACE ("RopllCmnConfig: FAIL no config for BitRate=%u\n", BitRate);
      return EFI_INVALID_PARAMETER;
    }
  } else {
    PHY_TRACE ("RopllCmnConfig: matched table entry BitRate=%u\n", Cfg->Bit_Rate);
  }
